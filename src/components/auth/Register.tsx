...
export function Register() {
  ...
  const handleEmailSubmit = () => {
    register({ name: fullName || 'Alima Alieva', email: formEmail || undefined });
    navigate('/');
  };

  const handlePhoneSubmit = () => {
    register({ name: phoneFullName || 'Alima Alieva', phone: formPhone || undefined });
    navigate('/');
  };

  const handleCodeSubmit = () => {
    register({ name: codeFullName || 'Alima Alieva' });
    navigate('/');
  };
  ...
  const tabs: { key: TabType; label: string; icon: typeof Mail }[] = [
    ...
    { key: 'code', label: '6x Code', icon: Hash },
  ];
        ...
        {activeTab === 'code' && (
          <div className="space-y-4">
            <input
              type="text"
              value={codeFullName}
              onChange={(e) => setCodeFullName(e.target.value)}
              placeholder={t('fullNamePlaceholder')}
              className="w-full px-4 py-3.5 bg-input-background border border-border rounded-xl text-sm focus:border-[#10b981] focus:outline-none transition-colors"
            />
